...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("S!d!madtv-Stb-v1")
        self.setGeometry(100, 100, 610, 560)

        self.settings = QSettings("S!d!madtv", "Stb-v1")
        self.profiles = []
        self.media_players = []
        self.favorites = []
        self.load_profiles()
        ...
        self.choose_player_button.setIcon(self.style().standardIcon(QStyle.SP_FileDialogDetailedView))
        media_player_layout.addWidget(self.choose_player_button)
        self.choose_player_button.clicked.connect(self.open_file_dialog)

        # Add media player dropdown
        self.media_player_combo = QComboBox()
        self.media_player_combo.setFixedWidth(150)
        media_player_layout.addWidget(self.media_player_combo)
        self.media_player_combo.currentTextChanged.connect(self.on_media_player_changed)

        # Add save player button
        self.save_player_button = QPushButton("Save")
        self.save_player_button.setIcon(self.style().standardIcon(QStyle.SP_DialogApplyButton))
        self.save_player_button.setFixedWidth(70)
        media_player_layout.addWidget(self.save_player_button)
        self.save_player_button.clicked.connect(self.save_current_player)

        # Load saved media players after combo box is created
        self.load_media_players()
        ...
        self.get_playlist_button.setIcon(self.style().standardIcon(QStyle.SP_MediaPlay))
        ...
        self.export_button = QPushButton("Export Portal")
        ...
        self.export_button.clicked.connect(self.export_portal)

        self.import_button = QPushButton("Import Portal")
        ...
        self.import_button.clicked.connect(self.import_portal)

        # Add favorite button
        self.add_favorite_button = QPushButton("❤️ Add to Favorites")
        self.add_favorite_button.setFixedWidth(150)
        export_import_layout.addWidget(self.add_favorite_button)
        self.add_favorite_button.clicked.connect(self.add_current_to_favorites)
        ...
        self.tabs = {}
        for tab_name in ["Live", "Movies", "Series", "Favorites", "Info"]:
            tab = QWidget()
            ...
            if tab_name == "Info":
                ...
                favorites_view = QListView()
                favorites_view.setEditTriggers(QAbstractItemView.NoEditTriggers)
                favorites_view.setMouseTracking(True)
                ...
                """)
                tab_layout.addWidget(favorites_view)

                favorites_model = QStandardItemModel(favorites_view)
                favorites_view.setModel(favorites_model)
                favorites_view.doubleClicked.connect(self.on_playlist_selection_changed)
                favorites_view.clicked.connect(self.on_playlist_selection_changed)
                favorites_view.setContextMenuPolicy(Qt.CustomContextMenu)
                favorites_view.customContextMenuRequested.connect(self.show_context_menu)

                self.tab_widget.addTab(tab, tab_name)
                self.tabs[tab_name] = {
                    "tab_widget": tab,
                    "playlist_view": favorites_view,
                    "playlist_model": favorites_model,
                    "playlist_data": [],
                ...
                playlist_view.setContextMenuPolicy(Qt.CustomContextMenu)
                playlist_view.customContextMenuRequested.connect(self.show_context_menu)
    ...
    def get_red_heart_icon(self):
        # Use standard dialog apply button for red heart indicator (no QPainter to prevent crashes)
        style = QApplication.style()
        return style.standardIcon(QStyle.SP_DialogApplyButton)

    def get_green_heart_icon(self):
        # Use standard dialog reset button for green heart indicator (no QPainter to prevent crashes)
        style = QApplication.style()
    ...
    def save_settings(self):
        ...
        self.save_media_players()
        self.save_profiles()
        self.save_favorites()

    def load_media_players(self):
        self.media_players = self.settings.value("media_players", [])
        if isinstance(self.media_players, str):
            self.media_players = []
        self.media_player_combo.clear()
        self.media_player_combo.addItem("Select Player...")
        for player in self.media_players:
            self.media_player_combo.addItem(player)

    def save_media_players(self):
        self.settings.setValue("media_players", self.media_players)

    def load_favorites(self):
        self.favorites = self.settings.value("favorites", [])
        if isinstance(self.favorites, str):
            self.favorites = []
        # Load favorites into the Favorites tab
        if "Favorites" in self.tabs:
            model = self.tabs["Favorites"]["playlist_model"]
            model.clear()
            for item in self.favorites:
                name = item.get("name", item.get("title", "Unknown"))
                qitem = QStandardItem(name)
                qitem.setData(item, Qt.UserRole)
                model.appendRow(qitem)
            self.tabs["Favorites"]["playlist_data"] = self.favorites

    def save_favorites(self):
        # Save favorites from the Favorites tab
        if "Favorites" in self.tabs:
            model = self.tabs["Favorites"]["playlist_model"]
            favorites = []
            for row in range(model.rowCount()):
    ...
    def add_current_to_favorites(self):
        # Get the current tab and selected item
        current_tab_name = self.tab_widget.tabText(self.tab_widget.currentIndex())
        
        if current_tab_name == "Favorites":
            QMessageBox.information(self, "Info", "Cannot add from Favorites tab.")
            return
        
        if current_tab_name not in self.tabs:
            return
        
        tab_info = self.tabs[current_tab_name]
        playlist_view = tab_info["playlist_view"]
        selected_indexes = playlist_view.selectedIndexes()
        
        if not selected_indexes:
            QMessageBox.warning(self, "Warning", "Please select an item to add to favorites.")
            return
        
        # Get the selected item
        index = selected_indexes[0]
        item = tab_info["playlist_model"].itemFromIndex(index)
        ...
        for fav in self.favorites:
            if fav.get("id") == data.get("id") or fav.get("cmd") == data.get("cmd"):
                QMessageBox.information(self, "Info", "This item is already in favorites.")
                return
        ...
        if "Favorites" in self.tabs:
            ...
            qitem = QStandardItem(name)
            qitem.setData(data, Qt.UserRole)
            model.appendRow(qitem)
            self.favorites.append(data)
            self.save_favorites()
            QMessageBox.information(self, "Success", f"{name} added to favorites!")
    ...
    def show_context_menu(self, position):
        ...
        index = playlist_view.indexAt(position)
        ...
        if current_tab_name == "Favorites":
            ...
            remove_action.triggered.connect(lambda: self.remove_from_favorites(data))
        else:
            # Show add to favorites option
            add_action = menu.addAction("❤️ Add to Favorites")
            add_action.triggered.connect(lambda: self.add_item_to_favorites(data))
        
        menu.exec_(playlist_view.viewport().mapToGlobal(position))

    def add_item_to_favorites(self, data):
        # Check if already in favorites
        for fav in self.favorites:
            if fav.get("id") == data.get("id") or fav.get("cmd") == data.get("cmd"):
                QMessageBox.information(self, "Info", "This item is already in favorites.")
        ...
        # Ensure data has required fields for playback
        if "cmd" not in fav_data and "stream_url" not in fav_data:
            # Try to construct cmd from other fields if available
            if "url" in fav_data:
                fav_data["cmd"] = fav_data["url"]
            elif "logo" in fav_data:
                fav_data["cmd"] = fav_data["logo"]
            else:
                # If still no cmd, add a placeholder to prevent errors
                fav_data["cmd"] = ""
        
        # Add to favorites
        if "Favorites" in self.tabs:
            model = self.tabs["Favorites"]["playlist_model"]
            name = fav_data.get("name", fav_data.get("title", "Unknown"))
            qitem = QStandardItem(name)
            qitem.setData(fav_data, Qt.UserRole)
            model.appendRow(qitem)
        ...
        if "Favorites" in self.tabs:
            ...
            for row in range(model.rowCount()):
                ...
                if item:
                    ...
                    if item_data and (item_data.get("id") == data.get("id") or item_data.get("cmd") == data.get("cmd")):
                        model.removeRow(row)
                        self.favorites = [fav for fav in self.favorites if not (fav.get("id") == data.get("id") or fav.get("cmd") == data.get("cmd"))]
                        self.save_favorites()
                        QMessageBox.information(self, "Success", "Removed from favorites!")
                        return

    def on_media_player_changed(self, player_path):
        if player_path and player_path != "Select Player...":
            self.media_player_input.setText(player_path)

    def save_current_player(self):
        current_player = self.media_player_input.text()
        if current_player and current_player not in self.media_players:
            self.media_players.append(current_player)
            self.media_player_combo.addItem(current_player)
            self.save_media_players()
            QMessageBox.information(self, "Success", "Media player saved!")
        elif current_player in self.media_players:
            QMessageBox.information(self, "Info", "Media player already saved.")
        else:
            QMessageBox.warning(self, "Warning", "Please enter a media player path first.")

    def export_portal(self):
        downloads_folder = os.path.join(os.path.expanduser("~"), "Downloads")
        default_path = os.path.join(downloads_folder, "S!d!madtv-Stb-v1_portal.json")
        
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Export Portal", default_path, "JSON Files (*.json)"
        )
        if file_path:
            try:
                export_data = {
                    "app_name": "S!d!madtv-Stb-v1",
                ...
                # Export all playlist data from the model
                for tab_name in ["Live", "Movies", "Series"]:
                    if tab_name in self.tabs:
                        model = self.tabs[tab_name]["playlist_model"]
                        playlist_items = []
                        for row in range(model.rowCount()):
                            item = model.item(row)
                            if item:
                        ...
                        export_data["playlists"][tab_name] = playlist_items
                ...
                if "Favorites" in self.tabs:
                    ...
                    for row in range(model.rowCount()):
                        ...
                        if item:
                            ...
                            if data:
                                favorites.append(data)
                    export_data["favorites"] = favorites
                
                with open(file_path, 'w', encoding='utf-8') as f:
                    json.dump(export_data, f, indent=2, ensure_ascii=False)
                QMessageBox.information(self, "Success", "Portal exported successfully!")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Failed to export portal: {e}")

    def import_portal(self):
        downloads_folder = os.path.join(os.path.expanduser("~"), "Downloads")
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Import Portal", downloads_folder, "JSON Files (*.json)"
        )
        if file_path:
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    import_data = json.load(f)
                if "hostname" in import_data:
                    self.hostname_input.setText(import_data["hostname"])
                if "mac_address" in import_data:
                    self.mac_input.setText(import_data["mac_address"])
                if "media_player" in import_data:
                    self.media_player_input.setText(import_data["media_player"])
                if "media_players" in import_data:
                    self.media_players = import_data["media_players"]
                    self.media_player_combo.clear()
                    self.media_player_combo.addItem("Select Player...")
                    for player in self.media_players:
                        self.media_player_combo.addItem(player)
                    self.save_media_players()
                if "threads" in import_data:
                    self.threads_input.setValue(import_data["threads"])
                if "profiles" in import_data:
                    self.profiles = import_data["profiles"]
                    self.save_profiles()
                
                # Import all playlist data
                if "playlists" in import_data:
                    for tab_name, items in import_data["playlists"].items():
                        if tab_name in self.tabs:
                            model = self.tabs[tab_name]["playlist_model"]
                            model.clear()
                            for item in items:
                                ...
                                qitem.setData(item, Qt.UserRole)
                                model.appendRow(qitem)
                            self.tabs[tab_name]["playlist_data"] = items
                            self.tabs[tab_name]["current_channels"] = items
                ...
                if "favorites" in import_data:
                    if "Favorites" in self.tabs:
                        model = self.tabs["Favorites"]["playlist_model"]
                        model.clear()
                        for item in import_data["favorites"]:
                            name = item.get("name", item.get("title", "Unknown"))
                            qitem = QStandardItem(name)
                            qitem.setData(item, Qt.UserRole)
                            model.appendRow(qitem)
                        self.tabs["Favorites"]["playlist_data"] = import_data["favorites"]
                        self.favorites = import_data["favorites"]
                
                QMessageBox.information(self, "Success", "Portal imported successfully!")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Failed to import portal: {e}")
    ...
    def update_info_tab(self, account_info, profile_info=None):
        ...
        created_at = js_account.get("created_at") or js_profile.get("created_at")
        ...
        if stalker_present:
            ...
            <b>Created At:</b> {created_at or 'N/A'}<br><br>
    ...
    def update_playlist_view(self, tab_name, scroll_position=0):
        ...
        if tab_info["current_category"] is None:
            for item in tab_info["playlist_data"]:
                name = item["name"]
                # Check if this category is in favorites
                is_favorite = any(
                    fav.get("id") == item.get("id") or fav.get("cmd") == item.get("cmd")
                    for fav in self.favorites
                )
                list_item = QStandardItem(name)
                list_item.setData(item, Qt.UserRole)
                list_item.setData("category", Qt.UserRole + 1)
                # Use colored heart icon: red for favorite, green for regular
                if is_favorite:
                    list_item.setIcon(self.get_red_heart_icon())
                else:
                    list_item.setIcon(self.get_green_heart_icon())
    ...
    def update_channel_view(self, tab_name, scroll_position=0):
        ...
        for channel in tab_info["current_channels"]:
            ...
            is_favorite = any(
                fav.get("id") == channel.get("id") or fav.get("cmd") == channel.get("cmd")
                for fav in self.favorites
            )
            ...
            # Use colored heart icon: red for favorite, green for regular
            if is_favorite:
                item.setIcon(self.get_red_heart_icon())
            else:
                item.setIcon(self.get_green_heart_icon())
    ...
    def on_playlist_selection_changed(self, index):
        ...
        # Handle favorites tab - play the selected item directly
        if current_tab == "Favorites":
            ...
            if item_data:
                try:
                    # Ensure item_data has required fields for playback
                    if "cmd" not in item_data and "stream_url" not in item_data:
                        logging.error(f"Favorite item missing cmd/stream_url: {item_data}")
                        QMessageBox.critical(self, "Error", "This favorite item is missing playback information.")
                        return
                    self.play_channel(item_data)
                except Exception as e:
                    logging.error(f"Error playing favorite: {e}")
                    QMessageBox.critical(self, "Error", f"Failed to play favorite: {e}")
            return
